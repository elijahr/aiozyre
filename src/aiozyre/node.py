...
from .xzyre import zyre_join, zyre_name, zyre_peers_by_group, zyre_recv, zyre_peer_address, zyre_peers, zyre_start, \
    zyre_peer_header_value, zpoller_new, zyre_peer_groups, zyre_own_groups, zyre_uuid, zyre_stop, zyre_whisper, \
    zyre_new, zyre_whispers, zyre_shouts, zyre_shout, zyre_socket, zyre_leave, zyre_set_header, zmsg_t, zpoller_wait, \
    zpoller_expired, zpoller_add
...
class Node(Threader):
    ...
    @property
    def name(self) -> str:
        return zyre_name(self._zyre)

    @property
    def uuid(self) -> str:
        return zyre_uuid(self._zyre)

    @property
    def socket(self):
        return zyre_socket(self._zyre)
    ...
    async def start(self):
        ...
        """
        self._zyre = zyre_new(self._name)
        if self._headers:
            for name, value in self._headers.items():
                self._set_header(name, value)
        self._start()
        self._poller = zpoller_new()
        zpoller_add(self._poller, self.socket)
        if self._groups:
    ...
    def _recv(self, timeout_ms: int = -1) -> Msg:
        if timeout_ms >= 0:
            sock = zpoller_wait(self._poller, timeout_ms)
            if sock is None:
                if zpoller_expired(self._poller):
                    raise Timeout
                else:
                    raise Stopped
        if not self._zyre:
            raise Stopped
        zmsg = zyre_recv(self._zyre)
        if zmsg is None:
            raise NodeRecvError
    ...
    def _whispers(self, peer: str, msg: str):
        zyre_whispers(self._zyre, peer, msg)

    def _whisper(self, peer: str, msg: zmsg_t):
        zyre_whisper(self._zyre, peer, msg)
    ...
    def _shouts(self, peer: str, msg: str):
        zyre_shouts(self._zyre, peer, msg)

    def _shout(self, peer: str, msg: zmsg_t):
        zyre_shout(self._zyre, peer, msg)
    ...
    def _peers_by_group(self, name: str) -> Set[str]:
        return zyre_peers_by_group(self._zyre, name)
    ...
    def _own_groups(self) -> Set[str]:
        return zyre_own_groups(self._zyre)
    ...
    def _peer_groups(self) -> Set[str]:
        return zyre_peer_groups(self._zyre)
    ...
    def _peer_address(self, peer: str):
        return zyre_peer_address(self._zyre, peer)
    ...
    def _peer_header_value(self, peer: str, name: str) -> str:
        return zyre_peer_header_value(self._zyre, peer, name)
    ...
    def _set_header(self, name: str, value: str):
        zyre_set_header(self._zyre, name, value)
